 ...
 * The portion before the first matched title becomes an implicit "正文/序" chapter.
 */
 ...
 * pages, broken at paragraph (newline) boundaries. This is the fallback for
 * books whose chapter headings can't be detected (the whole book would
 * otherwise be a single giant chapter) and for occasional huge chapters.
 * Pass targetChars <= 0 to disable.
 */
export function subdivideChapters(text: string, chapters: Chapter[], targetChars: number): Chapter[] {
  ...
  for (const ch of chapters) {
    ...
    while (start < ch.endOffset) {
      ...
      if (end < ch.endOffset) {
        // Extend to the next paragraph boundary so we don't cut mid-line.
        const nl = text.indexOf('\n', end);
        end = nl === -1 || nl >= ch.endOffset ? ch.endOffset : nl + 1;
      }
      // Merge a tiny trailing remainder into this piece.
