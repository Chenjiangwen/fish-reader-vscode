...
export class ReaderEngine {
   ...
   * title-only / empty chapters (e.g. a table-of-contents block or consecutive
   * headings). Returns the chapter's segments + its start + the next cursor.
   */
  ...
  prevChapter(): { segments: Segment[]; done: boolean } {
    // The chapter currently shown — fall back to the one just before the cursor.
    ...
    // Already at the first chapter: re-show the first chapter with content.
